...
import {
  type LockingInExpense,
  CATEGORY_COLORS,
  useLockingInExpenses,
  formatDate,
  formatAmount,
} from "@/components/locking-in-widget"

type SortKey = "expense" | "date" | "category" | "price"
...
function signedPrice(e: LockingInExpense): number {
  return (e.earned || 0) - (e.amount || 0)
}

function compareValues(a: LockingInExpense, b: LockingInExpense, key: SortKey, dir: SortDirection): number {
  let cmp = 0
  if (key === "price") {
...
function PriceCell({ exp }: { exp: LockingInExpense }) {
  if (exp.earned > 0) {
...
export default function LockingInPage() {
  const { allExpenses, loading, error, overviewStats, categoryColorMap } = useLockingInExpenses()
  ...
  }, [allExpenses, selectedCategories, sortKey, sortDirection])
  ...
  const showingFiltered = selectedCategories.size > 0
            ...
            <div className="border border-border rounded-lg px-4 py-3 mb-4 bg-muted/5">
              <div className="flex items-start gap-4">
                <div className="flex-1 min-w-0 space-y-2">
                  <div>
                    <div className="text-[10px] uppercase tracking-wider text-muted-foreground mb-0.5">Spent</div>
                    <div className="text-2xl font-bold text-foreground font-mono tabular-nums leading-none">
                      −{formatAmount(overviewStats.total)}
                    </div>
                  </div>
                  <div>
                    <div className="text-[10px] uppercase tracking-wider text-muted-foreground mb-0.5">Earned</div>
                    <div className="text-2xl font-bold text-emerald-500 font-mono tabular-nums leading-none">
                  ...
                  <div className="pt-1 border-t border-border/50">
                    <div className="flex items-baseline justify-between gap-2">
                      ...
                      <span className={`text-sm font-semibold font-mono tabular-nums ${overviewStats.net >= 0 ? "text-emerald-500" : "text-red-500"}`}>
